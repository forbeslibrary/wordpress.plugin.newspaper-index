...
class Newspaper_Index_Admin {
  ...
  */
  public function validate_and_save( $post_id ){
    ...
    // Update custom field
    if (isset($_POST[$this->data['post_type']])) {
      $custom_fields = $_POST[$this->data['post_type']];
      foreach($custom_fields as $field_name => $content) {
        $custom_fields[$field_name] = trim($content);
      }
      update_post_meta($post->ID, $this->data['post_type'], $custom_fields);
    }
  ...
  */
  ...
  public function editbox_metadata(){
    ...
    ?>
    <table class="<?php echo $this->data['post_type']; ?>-metadata-table" >
      <?php foreach ($this->data['custom_fields'] as $custom_field => $input_type): ?>
        <tr>
          <td><label
            class="<?php echo $this->data['post_type']; ?>-metadata-label"
            for="<?php echo "{$this->data['post_type']}[$custom_field]"; ?>"
            >
          ...
          <td>
            <?php switch ($input_type):
              case "checkbox" ?>
              <input
                name="<?php echo "{$this->data['post_type']}[$custom_field]"; ?>"
                class="<?php echo $this->data['post_type']; ?>-input"
                value="no"
                type="hidden"
                >
              <input
                id="<?php echo "{$this->data['post_type']}[$custom_field]"; ?>"
                name="<?php echo "{$this->data['post_type']}[$custom_field]"; ?>"
                class="<?php echo $this->data['post_type']; ?>-input"
